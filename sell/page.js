...
export default function SellPage() {
  ...
  const [status, setStatus] = useState(null); // { mode: 'pending'|'success'|'error', message }
  ...
  async function loadProducts() {
    ...
    if (error || !data || !data.length) {
      // Falls back silently so the page still works while Supabase products
      // hasn't been seeded yet — see supabase.sql.
      setProducts(FALLBACK_PRODUCTS);
    } else {
      setProducts(data);
    }
  ...
  function addToCart(product) {
    ...
    setCart((prev) => {
      ...
      if (currentQty >= product.stock) return prev; // can't add past live stock
      if (existing) {
        return prev.map((l) =>
  ...
  async function handleCheckout() {
    ...
    const fulfilled = []; // [{ id, name, price, qty, stockAfter }]
    const unavailable = []; // [{ name, reason }]

    // Decrement stock one line at a time, re-checking live stock each time
    // so two cashiers checking out at once can't oversell the same item.
    ...
    // Checkout already succeeded at this point — Telegram alerts run
    // separately (fire-and-forget) and can never turn a successful sale
    // into an error, even if the Telegram call fails or times out.
